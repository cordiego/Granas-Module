...
class PowerScaling:
    ...
    def print_table(self):
        """Print formatted scaling table."""
        results = self.compute_all()

        print()
        print("═" * 80)
        print(" GRANAS MODULE — Power Scaling: Home → Continent")
        print(f" Module: {self.module.width_m}m × {self.module.height_m}m │ "
              f"{self.module.peak_power_W:.0f} W │ "
              f"{self.module.annual_energy_kWh:.0f} kWh/yr │ "
              f"CF={self.module.capacity_factor}")
        print("─" * 80)
        print(
            f"{'':17s} │ {'Modules':>10s}        │ "
            f"{'Power':>12s}    │ {'Consumption':>10s}"
        )
        print("─" * 80)
...
if __name__ == "__main__":
    ...
    print("\n  ── Detailed Breakdown ──")
    for r in scaling.compute_all():
        print(f"\n  {r.scale.emoji} {r.scale.name}:")
        print(f"    Consumption:    {r.scale.annual_kWh:,.0f} kWh/yr")
        print(f"    Modules:        {r.modules_needed:,}")
        print(f"    Total Area:     {r.total_area_m2:,.0f} m²"
              f" ({r.total_area_ha:,.1f} ha"
              f" / {r.total_area_km2:,.2f} km²)")
        print(f"    Installed:      {r.total_power_MW:,.1f} MW"
              f" ({r.total_power_GW:,.3f} GW)")
        print(f"    Weight:         {r.total_weight_tonnes:,.0f} tonnes")
