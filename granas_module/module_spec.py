...
Cell-Level (single 21cm × 34cm tandem sub-cell):
  Voc_cell  = 1,135 mV  (1,100 mV base + 35 mV green cooling)
  Jsc_cell  = 36.3 mA/cm²
  FF        = 0.80
  PCE       = ~33%
  Active    = 624 cm²
...
Module-Level (50S × 2P configuration):
  Voc_module  = 56.75 V   (50 × 1.135 V)
  Isc_module  = 45.3 A    (2 × 22.65 A)
  Pmax        = 2,059 W   (STC)
  Annual      = 3,965 kWh (CF=0.22, Mexico)
...
class GranasProductionModule:
    ...
    def compute(self) -> "GranasProductionModule":
        ...
        # Cross-check: P = Voc × Isc × FF
        p_check = self.module_voc_V * self.module_isc_A * self.module_ff
        # These should be very close (rounding only)
